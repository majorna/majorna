...
suite('db', () => {
  ...
  test.only('giveMiningReward', async () => {
    ...
    await db.setBlockInfo(blockInfo)

    // mine the block
    const hashBase64 = blockUtils.mineBlock(nextBlock.header)
    ...
    // collect reward
    const rewardTx = await db.giveMiningReward(to, nextBlock.header.nonce)
