...
suite('route: miners', () => {
  test('joinMiners', async () => {
    ...
    const res = await testData.users.u1Request.post('/peers', { id, lat: 0, lon: 123 })
    assert(res.status === 201)
    assert(res.data.miners.find(m => m.lon === 123))

    // update location
    const res2 = await testData.users.u1Request.post('/peers', { id, lat: 1.234, lon: 0 })
    assert(res2.status === 201)
    assert(res2.data.miners.find(m => m.lat === 1.234))

    // invalid request
    const res3 = await testData.users.u1Request.post('/peers', { id, lon: 3.4 })
