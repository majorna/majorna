    ...
    {!props.mjMetaDoc ? <div className="mj-box center-all spinner"/> :
      <div className="mj-box home-stats-grid">
        <div className="flex-column center-all">
          <strong className="is-size-5">${props.mjMetaDoc.val}</strong>
          Current Price
        </div>
        <div className="flex-column center-all">
          <strong className="is-size-5">${fm(props.mjMetaDoc.marketCap * props.mjMetaDoc.val)}</strong>
          Market Cap.
        </div>
        <div className="flex-column center-all">
          <strong className="is-size-5">{fm(props.mjMetaDoc.marketCap / 1000000)}M mj</strong>
          Current Supply
        </div>
        <div className="flex-column center-all">
          <strong className="is-size-5">{fm(props.mjMetaDoc.maxSupply / 1000000)}M mj</strong>
          Max Supply
        </div>
        <div className="flex-column center-all">
          <strong className="is-size-5">${fm(props.mjMetaDoc.monthly.txVolume * props.mjMetaDoc.val)}</strong>
          Tx Volume (monthly)
        </div>
        <div className="flex-column center-all">
          <strong className="is-size-5">{props.mjMetaDoc.userCount}</strong>
          Total Accounts
        </div>
      </div>
    }
